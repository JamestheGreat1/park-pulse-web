...
function ensureSeasonalLayer() {
  ...
  layer.innerHTML = Array.from({ length: 28 }, (_, index) =>
    `<span class="seasonal-particle" style="--particle-index:${index};--particle-left:${(index * 37 + 11) % 101}%;--particle-delay:-${(index * 1.37).toFixed(2)}s;--particle-duration:${(11 + (index % 7) * 1.9).toFixed(1)}s"></span>`
  ).join("");
