...
(async () => {
  ...
  try {
    ...
    const cached = await page.evaluate(async () => (await (await caches.open('parkpulse-1.6.3-shell')).keys()).map(r => r.url));
    for (const module of ['app', 'api', 'data', 'store', 'push', 'config']) assert(cached.some(url => url.endsWith(`/assets/js/${module}.js?v=1.6.3`)));
