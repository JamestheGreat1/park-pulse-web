...
test('service worker precaches all modules and never caches HTTP failures', async () => {
  ...
  await pending;
  for (const name of ['app', 'api', 'data', 'store', 'push', 'config']) assert(precached.includes(`./assets/js/${name}.js?v=1.8.5`));
  handlers.fetch({ request: { method: 'GET', url: 'https://app.example/assets/js/data.js?v=1.8.5' }, respondWith: promise => { pending = promise; } });
