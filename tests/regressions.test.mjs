...
test('service worker precaches all modules and never caches HTTP failures', async () => {
  ...
  await pending;
  for (const name of ['app', 'api', 'data', 'store', 'push', 'config']) assert(precached.includes(`./assets/js/${name}.js?v=1.8.11`));
  handlers.fetch({ request: { method: 'GET', url: 'https://app.example/assets/js/data.js?v=1.8.11' }, respondWith: promise => { pending = promise; } });
...
test('desktop ride sheet uses the denser 1.8.11 layout', () => {
  const css = fs.readFileSync(new URL('../assets/css/app.css', import.meta.url), 'utf8');
