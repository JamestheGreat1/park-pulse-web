...
test('service worker precaches all modules and never caches HTTP failures', async () => {
  ...
  await pending;
  for (const name of ['app', 'api', 'data', 'store', 'push', 'config']) assert(precached.includes(`./assets/js/${name}.js?v=1.8.4-preview.19`));
  handlers.fetch({ request: { method: 'GET', url: 'https://app.example/assets/js/data.js?v=1.8.4-preview.19' }, respondWith: promise => { pending = promise; } });
